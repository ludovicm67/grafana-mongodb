...
Two dependencies are deliberately kept in step with that scaffolding rather than at their
latest version, and are ignored by Dependabot:

- `@grafana/eslint-config` is pinned to v9, because v10 dropped the `./flat.js` entry point
  that `.config/eslint.config.mjs` imports.
- `@stylistic/eslint-plugin-ts` is required as a peer dependency by `@grafana/eslint-config` v9.

Both can be moved forward once `create-plugin` adopts `@grafana/eslint-config` v10.
